...
前章では、フッターナビゲーションを共通レイアウトへ組み込み、四つのページで共有しました。この章では、画面上部へ表示する共通ヘッダーを作成します。

実装を始める前に、コンポーネントの配置を整理します。このプロジェクトではFeatureベースの構成を採用しているため、ヘッダー固有のコンポーネントは`features/header`へ配置します。一方、shadcn/uiから追加するAvatarのように、特定の機能へ依存しないUI部品は`shared/ui`へ配置します。
...
この章では、Feature固有のコンポーネントと共有部品の配置を整理しました。shadcn/uiの生成先を`shared/ui`へ変更し、既存のButtonと`cn`もそれぞれ`shared/ui`と`shared/lib`へ移動しました。

さらに、`AppLogo`と`UserAvatar`を`Header`で組み合わせ、shadcn/uiのAvatarを使った共通ヘッダーを作成しました。`_app.tsx`へ組み込むことで、同じヘッダーを四つのページへ表示しています。

これで、画面上部のヘッダー、ページごとの内容、画面下部のフッターナビゲーションというアプリケーション全体の枠組みが整いました。次の章では、ダッシュボードへ予算情報などの内容を追加していきます。
