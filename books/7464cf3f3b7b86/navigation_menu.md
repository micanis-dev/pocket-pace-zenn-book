...
前章で4ページを移動できるようになりました。ただ、今は各ページへ文字のリンクを直接書いただけなので、Figmaで描いたフッターナビゲーションにはまだ遠い状態です。

今回は、見た目を確認しやすい小さな部品に分けながら、リンクをナビゲーションらしい形にします。コンポーネントをどう分ければよいかまだ手探りなので、それぞれの担当を考えながら進めます。4ページで共有する作業は次章で行います。

## デザインをコンポーネントへ分ける

Figmaで作ったデザインをそのまま一つの大きなコンポーネントへ変換するのではなく、役割を考えていくつかの部品へ分けます。今回は、4章で作成した画面下部のナビゲーションを次のように分けました。

![フッターナビゲーションのコンポーネント分割](/images/7464cf3f3b7b86/footer-figma.png =500x)
...
この部分は、[[React + Typescript + MUI] アイコン付きテキストナビゲーションの作り方](https://zenn.dev/holypotter0304/articles/a0a5eaddbf0634)を参考にしました。記事ではMUIを使っていますが、「リンク先、表示する文字、アイコンを配列へまとめ、`map`でナビゲーションを作る」という考え方はLucideでも同じように使えました。

また、アイコンを配列へ入れるときの型は、[React特有の型定義](https://zenn.dev/masato24524/articles/9c772857328066)にある`React.ComponentType`の説明が参考になりました。アイコンは画像ファイルではなくReactコンポーネントなので、`ComponentType`を使うと「コンポーネントそのものを値として受け取る」ことを型で表せる、と理解しました。今回のコードでは、あとから`className`を渡せるようにするため、後半で`ComponentType<{ className?: string }>`へ変更します。
...
![コンポーネントの範囲を境界線で確認する](/images/7464cf3f3b7b86/footer-img01.png =404x)
...
![外側からスタイルを渡した途中経過](/images/7464cf3f3b7b86/footer-img02.png =404x)
...
今回は、仮の境界線を付けて`LabeledIcon`、`PillGroup`、`FooterNav`の範囲を確認しました。線だらけで見た目はよくありませんが、どの部品がずれているのか分かりやすくなりました。

`classNames`と`cn`を使う書き方は初めてで少し悩みましたが、同じ部品を場所に合わせて調整できる形にできました。まだダッシュボードに置いただけなので、次は4ページで共有し、画面下へ固定します。
