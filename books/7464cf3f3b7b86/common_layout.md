...
前章でフッターナビゲーションの形ができました。しかし、今はダッシュボードに直接置いているため、ほかの3ページには表示されません。今回は4ページで同じナビゲーションを使えるようにし、画面の下へ固定します。
...
![完成したフッターナビゲーション](/images/7464cf3f3b7b86/footer-fin.png =436x)
...
`_app.tsx`へページの表示領域と`FooterNav`をまとめたことで、4ページすべてに同じナビゲーションが出るようになりました。各ページへ同じコードを書かずに済む理由も、実際にファイルを動かしてみて少し理解できました。

画面下へ固定すると本文と重なる問題がありましたが、ナビゲーションの寸法をCSS変数にして、その分の余白を取ることで解決できました。Safe Areaは今回初めて知ったので、実機に近い環境でも改めて確認したいです。
