...
前章では、Devboxとdirenvを使って、Node.js 24とpnpmをプロジェクトごとの環境で利用できるようにしました。この章では、その環境を使ってReact Router v7のプロジェクトを作成します。さらにTailwind CSSとshadcn/uiを導入し、開発サーバーの起動と本番用ビルドができるところまで確認します。

## 使用するツール

この章で使用するReact Router、Tailwind CSS、shadcn/uiについて、それぞれの役割を簡単に紹介します。
...
この章では、前章で構築したNode.jsとpnpmの環境を使い、React Router v7のプロジェクトを作成しました。さらにTailwind CSSを追加してshadcn/uiを初期化し、開発サーバーの起動と本番用ビルドを確認しました。

これで、Figmaで設計した各画面をReactで実装する準備が整いました。次の章では、各画面とURLを対応させるルーティングを設定します。
