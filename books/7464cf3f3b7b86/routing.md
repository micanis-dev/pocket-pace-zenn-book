...
前章まででReact Routerのプロジェクトが動くようになりました。ここから、Figmaで考えた画面を少しずつ実装していきます。

最初はルーティングです。私は「リンクを押したら別の画面を出すもの」くらいに考えていましたが、URLへの直接アクセスやブラウザの戻る・進むも関係することを知りました。自分で全部を扱うのは難しそうなので、React Routerを使います。
...
`@react-router/fs-routes`を追加し、ファイル名とURLが対応するようにできました。4ページはまだ中身が同じですが、`NavLink`でURLが切り替わるところまでは確認できました。

見た目に変化がなくて少し不安になりましたが、今はURLが変われば成功です。次からは、画面に実際の部品を置いていきます。
